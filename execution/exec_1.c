...
/*   Updated: 2023/06/18 10:40:26 by aachfenn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_1(t_minishell	*mini, t_cmd	*cmd, char	**env)
{
	...
	prep->i = 0;
	prep->pid = malloc(sizeof(int *) * cmd->general_info->cmd_nb + 1);
...
// 	cmd->general_info->std_in = dup(0);
// 	cmd->general_info->std_out = dup(1);
// 	prep = malloc(sizeof(t_prep) + 1024 * 1024);
// 	pid = malloc(sizeof(int) * cmd->general_info->cmd_nb);
// 	prep->nb = cmd->general_info->cmd_nb;
// 	prep->i = 0;
// 	while (prep->i < cmd->general_info->cmd_nb - 1)
// 	{
// 		if (cmd[prep->i].args[0] && built_in_cmd_3(mini, &cmd[prep->i], env))
